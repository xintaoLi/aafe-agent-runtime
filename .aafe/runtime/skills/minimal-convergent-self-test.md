...
Companion: `.aafe/runtime/skills/architecture-impact-test-forecast.md` (impact) → this skill (tests) → **完成后** 若任务有关联 TAPD 单 → `.aafe/runtime/skills/tapd-submit-backfill.md`.
...
Path: install-root `test/`（与 `.aafe.config.json` / `.aafe/runtime` 同级；无则创建）。
...
1. 若任务过程中**有关联 TAPD 单**且 `tapd.enabled`：Read `.aafe/runtime/skills/tapd-submit-backfill.md` Phase B 起（按 submit.cli 执行 Commit/PR → 询问 TAPD 回填）
