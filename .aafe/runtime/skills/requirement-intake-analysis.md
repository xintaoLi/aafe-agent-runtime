...
Rule: `.aafe/runtime/rules/requirement-intake-analysis.mdc`

Post-implementation (unchanged): `.aafe/runtime/rules/task-completion-impact.mdc` → `.aafe/runtime/rules/tapd-submit-backfill.mdc`
...
3. **未匹配** → 按 `.aafe/runtime/skills/tapd-submit-backfill.md`「GTM Task Start」执行：  
...
1. Read `.aafe/runtime/skills/memory-recaller.md`
2. Search:
   - `.aafe/runtime/memory/experience.md`
   - `.aafe/runtime/memory/learnings.jsonl`
   - `.aafe/runtime/memory/decisions.md`, topic files if relevant
...
- Non-trivial frontend: then `.aafe/runtime/runtime/engine.md`, router, pipelines, gates as usual
