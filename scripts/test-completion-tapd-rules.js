...
assert.match(projectTapd, /TAPD 来源任务/);
assert.match(projectTapd, /回填失败.*不阻塞后续独立步骤/);
assert.match(projectTapd, /状态映射逐步流转到 doing/);
assert.match(projectTapd, /临时注入/);
assert.match(projectTapd, /ID 不匹配/);
assert.match(projectTapd, /不得因当前分支已有相关提交或存在未提交改动而放行继续需求分析、设计还原或实现/);
...
assert.match(tapdSkill, /准备进入需求分析、设计还原或写代码前/);
...
assert.match(tapdSkill, /TAPD-origin default/);
assert.match(tapdSkill, /Final aggregation/);
...
assert.match(projectReq, /不得因当前分支已有相关提交或未提交改动而放行继续分析、设计还原或实现/);
...
assert.match(reqSkill, /禁止继续 Phase 1、设计还原或写代码/);
assert.match(reqSkill, /尚未开始需求分析、设计还原或写代码/);
...
assert.match(spine, /需求\/设计分析或写代码前/);
